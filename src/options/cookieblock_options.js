...
const removeItemFromList = async function (removedDomain, listItem, storageID) {
    let exceptions_list = listItem.parentElement;
    ...
    exceptions_list.removeChild(listItem);
}
...
const appendItemToList = async function (addDomain, listID, storageID) {
    ...
    let text_node = document.createTextNode(addDomain);

    text_div.appendChild(text_node);
    text_div.class = "text_content";
...
const setupLocalization = function () {
    ...
    setStaticLocaleText("general_settings", "headerGeneralSettings");
    ...
    setStaticLocaleText("extra_opts_legend", "headerAdvancedSettings");
...
/**
 * Handler of user exceptions.
 * For both Domain Exceptions and Known Cookies
 */
function userExceptionsHandler() {
...
/**
 * Checkboxes for consent purposes.
 * Select the types of cookies you want to permit. You can change these settings anytime.
 */
async function consentPurposesCheckboxes() {
    let policy = await getStorageValue(chrome.storage.sync, "cblk_userpolicy");
    const necessaryCheckbox = document.getElementById("nec_checkbox");
...
/**
 * Listener for the reset storage button.
 * This button deletes the current cookie history, does not remove cookies from the browser itself!
 */
 ...
 * This button restores the default settings of the extension.
 */
...
/**
 * Listener for the classify all cookies button.
 * Categorize and Remove Stored Cookies
 * This button will categorize all cookies currently stored in the browser,
 * remove those rejected by the user policy, and close the initial setup tab.
 */
function classifyAllCookieButton() {
...
/**
 * Creates page for the JSON documents.
 * @param {String} inMSG document name
 */
...
/**
 * Listener for the JSON document buttons.
 * Click on any of the labels to receive a snapshot of the current cookie history
 */
function jsonDocsListeners() {
...
const updateSelectionsOnStorageChanged = function (changes, area) {
    ...
    if (area === "sync") {
        ...
        if (changedItems.includes("cblk_exglobal")) {
            emptyAndRestoreList(changes["cblk_exglobal"].newValue,
                "cblk_exglobal", "website_exceptions");
        }
...
const setupSettingsPage = async function () {
    ...
    submitHandleListener();

    // history consent checkbox
    const histCheckbox = document.getElementById("history-consent-checkbox");
    let hist_consent = await getStorageValue(chrome.storage.sync, "cblk_hconsent");
    histCheckbox.checked = hist_consent;
    // hide the statistics section if the user has not consented to history tracking
    const statisticsSection = document.getElementById("stats-div");
    statisticsSection.hidden = !hist_consent; histCheckbox.addEventListener("click", (ev) => {
        setStorageValue(histCheckbox.checked, chrome.storage.sync, "cblk_hconsent");
    });

    await consentPurposesCheckboxes();
    ...
    resetStorageButton();
    resetSettingsButton();
    classifyAllCookieButton();

    // Predefined Cookie Categories checkbox
    ...
    // User-defined Cookie Categories checkbox
    ...
    addExcClickListener("website_excepts_submit", "website_excepts_input",
        "cblk_exglobal", "website_exceptions", "wexcepts-error");
